...
function buildAdminNav(ctx, activePage) {
  const base = `/frontend/admin`;
  const hash = `#/${ctx.org}/${ctx.name}`;
  const pages = [
    { id: "index",      label: "Hub",       href: `${base}/index.html${hash}` },
    { id: "inspection", label: "Inspection",href: `${base}/inspection.html${hash}` },
    { id: "track",      label: "Track",     href: `${base}/track.html${hash}` },
    { id: "results",    label: "Officials", href: `${base}/results.html${hash}` },
    { id: "cars",       label: "Cars",      href: `${base}/cars.html${hash}` },
    { id: "announcer",  label: "Screen",    href: `${base}/announcer.html${hash}` },
  ];
  return pages.map(p =>
